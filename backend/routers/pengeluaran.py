...
from utils import fmt_float, fmt_date, imgbb_upload
...
async def tambah_pengeluaran(
    ...
    try:
        ...
        log.info(f"[Pengeluaran] Baru: {deskripsi} (Rp {jumlah:,.0f})")
...
async def hapus_pengeluaran(pid: str, user=Depends(req_owner), cur=Depends(get_db)):
    try:
        ...
        log.error(f"[Pengeluaran] Gagal menghapus pengeluaran {pid}: {e}")
        raise HTTPException(500, "Gagal menghapus pengeluaran.")
