...
async def tambah_kendaraan(body: KendaraanIn, user=Depends(req_owner), cur=Depends(get_db)):
    try:
        ...
        log.info(f"[Kendaraan] Baru: {body.nama_kendaraan} ({body.nomor_plat})")
...
async def hapus_kendaraan(kid: str, user=Depends(req_owner), cur=Depends(get_db)):
    try:
        ...
        log.info(f"[Kendaraan] Dihapus: {row['nama_kendaraan']} ({kid})")
        return {"message": f"Kendaraan '{row['nama_kendaraan']}' berhasil dihapus."}
    except HTTPException:
        raise
    except Exception as e:
        log.error(f"[Kendaraan] Gagal menghapus kendaraan {kid}: {e}")
        raise HTTPException(500, "Gagal menghapus kendaraan.")
